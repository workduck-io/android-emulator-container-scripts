...
clean_up() {
  ...
  run rm -rf /tmp/*
  run rm -rf ${ANDROID_AVD_HOME}/Pixel3.avd/*.lock
...
initialize_data_part() {
  ...
  if  is_mounted /data; then
    ...
    echo "path=${ANDROID_AVD_HOME}/Pixel3.avd" > ${ANDROID_AVD_HOME}/Pixel3.ini
  else
...
if [ ! -z "${AVD_CONFIG}" ]; then
  ...
  echo "${AVD_CONFIG}" >>"${ANDROID_AVD_HOME}/Pixel3.avd/config.ini"
fi
...
var_append LAUNCH_CMD -avd Pixel3
